...
import { UpdateBusinessUser, GetBusinessUserById,UpdateBusinessUserPass } from "../api/BusinessUserController";
...
const BusinessUpdateForm = () => {
  ...
  const handleUpdatePass=async() => {
    ...
    if(prevPass.length>7 && newPass.length>7){
      ...
      if(res === 1){alert("סיסמה התעדכנה בהצלחה!")}
        ...
        {user === null || user === undefined ? <Loader /> :
          ...
          <div dir="rtl" className="flex flex-col mt-11  p-5">
            <div className="flex justify-around items-center mb-5 ">
              ...
              <input className="w-80 p-2" type="text" defaultValue={user.businessLicense} onChange={(e) => { setBusinessLicense(e.target.value) }} />
            </div>
            <div className="flex justify-around items-center mb-5 ">
              <h1 className="w-28 text-xl font-medium">שם העסק</h1>
              <input className="w-80 p-2" type="text" defaultValue={user.businessName} onChange={(e) => { setBusinessName(e.target.value) }} />
            </div>

            <div className="flex justify-around items-center mb-5 ">
              <h1 className="w-28 text-xl font-medium">כתובת עסק</h1>
              <input className="w-80 p-2" type="text" defaultValue={user.businessAddress} onChange={(e) => { setBusinessAddress(e.target.value) }} />
            </div>
            <div className="flex justify-around items-center mb-5 ">
              <h1 className="w-28 text-xl font-medium">טלפון</h1>
              <input className="w-80 p-2" type="text" defaultValue={user.businessPhone} onChange={(e) => { setBusinessPhone(e.target.value) }} />
            </div>
            <div className="flex justify-around items-center mb-5 ">
              <h1 className="w-28 text-xl font-medium">תיאור העסק</h1>
              <input className="w-80 p-2" type="text" defaultValue={user.businessDescription} onChange={(e) => { setBusinessDescription(e.target.value) }} />
            </div>
